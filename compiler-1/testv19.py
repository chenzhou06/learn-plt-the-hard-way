...
import unittest
import io
from schemev19 import *         # pylint: disable=unused-wildcard-import, wildcard-import
...
class TestApply(unittest.TestCase):
    "Tests for apply procedure."
    def setUp(self):
        self.applyexp = SCMRead(io.StringIO("(apply + '(1 2 3))"))
        self.applyop = make_primitive_proc(add_proc)
        self.arguments = list_of_values(operands(self.applyexp),
                                        SCMTheGlobalEnvironment)
        self.applyoprands = SCMRead(io.StringIO("(1 2 3)"))

    def test_apply_operator(self):
        "Test for `apply_operator`."
        self.assertEqual(apply_operator(self.arguments), self.applyop)

    def test_apply_operands(self):
        "Test for `apply_operands`."
        self.assertEqual(apply_operands(self.arguments), self.applyoprands)

    def test_eval(self):
        "Test for evaluation."
        self.assertEqual(SCMEval(self.applyexp, SCMTheGlobalEnvironment),
                         make_fixnum(6))


class TestEval(unittest.TestCase):
    "Tests for eval procedure."
    def setUp(self):
        self.evalexp = SCMRead(io.StringIO("(eval '(+ 1 1) (environment))"))
        self.arguments = list_of_values(operands(self.evalexp), SCMTheGlobalEnvironment)
        self.exp = SCMRead(io.StringIO("(+ 1 1)"))
        self.env = make_environment()

    def test_interaction_env(self):
        "Test for `interaction_environment_proc`."
        self.assertEqual(interaction_environment_proc(self.evalexp),
                         SCMTheGlobalEnvironment)

    def test_setup_environment(self):
        "Test for `setup_environment`."
        self.assertEqual(SCMCar(setup_environment()), SCMCons(SCMTheEmptyList, SCMTheEmptyList))
        self.assertEqual(SCMCdr(setup_environment()),
                         SCMTheEmptyEnvironment)

    def test_null_environment(self):
        "Test for `null_environment`."
        self.assertEqual(null_environment_proc(self.evalexp),
                         setup_environment())

    def test_eval_exp(self):
        "Test for `eval_expression`."
        self.assertEqual(eval_expression(self.arguments),
                         self.exp)

    def test_eval_env(self):
        "Test for `eval_environment`."
        self.assertEqual(eval_environment(self.arguments), self.env)

    def test_eval(self):
        "Test for evaluation."
        self.assertEqual(SCMEval(self.evalexp, SCMTheGlobalEnvironment),
                      make_fixnum(2))
