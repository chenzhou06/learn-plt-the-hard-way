...
def apply_proc(_):
    ...
    raise Exception(
        "Illegal state: The body of the apply primitive procedure should not execute")
...
SCMLetSymbol = make_symbol("let")
SCMAndSymbol = make_symbol("and")
SCMOrSymbol = make_symbol("or")
...
def make_application(operator_obj, operands_obj):
    """Make an application.
    ...
    ```
    """
    return SCMCons(operator_obj, operands_obj)
...
def null_environment_proc(_):
    "Returns an initial environment."
    return setup_environment()


def interaction_environment_proc(_):
    "Returns the initial global environment."
    return SCMTheGlobalEnvironment


def environment_proc(_):
    "A scheme procedure return a initialized environment."
    return make_environment()


def eval_proc(_):
    "A dummy procedure for eval, should never be executed."
    raise Exception(
        "Illegal state: The body of the eval primitive procedure should not execute.")


def populate_environment(env):
    "Populate primitive procedures into env."

    def add_procedure(scheme_name, py_name):
        "Helper function for add procedure."
...
SCMTheGlobalEnvironment = make_environment()
...
def is_let(exp):
    "Let predicate."
    return is_tagged_list(exp, SCMLetSymbol)


def let_bindings(exp):
    "Extact bindings from let expression."
    return SCMCar(SCMCdr(exp))


def let_body(exp):
    "Extract body from a let expression."
    return SCMCdr(SCMCdr(exp))


def binding_parameter(binding):
    "Return the parameter from a binding of a let exp."
    return SCMCar(binding)


def binding_argument(binding):
    "Return the value of that binding."
    return SCMCar(SCMCdr(binding))


def bindings_parameters(bindings):
    "Return all parameters from bindings."
    return SCMTheEmptyList if is_the_empty_list(bindings) else \
        SCMCons(binding_parameter(SCMCar(bindings)),
                bindings_parameters(SCMCdr(bindings)))


def bindings_arguments(bindings):
    "Return all arguments."
    return SCMTheEmptyList if is_the_empty_list(bindings) else \
        SCMCons(binding_argument(SCMCar(bindings)),
                bindings_arguments(SCMCdr(bindings)))


def let_parameters(exp):
    "Return parameters of a let exp."
    return bindings_parameters(let_bindings(exp))


def let_arguments(exp):
    "Return arguments of a let exp."
    return bindings_arguments(let_bindings(exp))


def let_to_application(exp):
    "Make an application out of a let exp."
...
def is_and(exp):
    "Return true if is (and exps)."
    return is_tagged_list(exp, SCMAndSymbol)


def and_tests(exp):
    "Extract all tests from a and expression."
    return SCMCdr(exp)


def is_or(exp):
    "Return true if is (or exps)."
    return is_tagged_list(exp, SCMOrSymbol)


def or_tests(exp):
    "Extract tests from an or expression."
    return SCMCdr(exp)


def apply_operator(arguments):
    "Extract operator from apply procedure."
    return SCMCar(arguments)


def prepare_apply_operands(arguments):
    "Make operands for apply procedure."
    if is_the_empty_list(SCMCdr(arguments)):
        return SCMCar(arguments)
    else:
        return SCMCons(SCMCar(arguments),
                       prepare_apply_operands(SCMCdr(arguments)))


def apply_operands(arguments):
    "Extract operands from apply procedure."
    return prepare_apply_operands(SCMCdr(arguments))


def eval_expression(arguments):
    "Extract expression from an eval procedure."
    return SCMCar(arguments)


def eval_environment(arguments):
    "Extract environment from an eval procedure."
    return SCMCar(SCMCdr(arguments))
...
def SCMEval(exp, env):
    if is_self_evaluating(exp):
        ...
        if is_the_empty_list(exp):
            return SCMTrue
        while not is_last_exp(exp):
            result = SCMEval(first_exp(exp), env)
            if is_false(result):
                return result
            exp = rest_exp(exp)
        exp = first_exp(exp)
        ...
        if is_primitive_proc(procedure) and procedure.data.value == eval_proc:
            exp = eval_expression(arguments)
            env = eval_environment(arguments)
            return SCMEval(exp, env)

        if is_primitive_proc(procedure) and procedure.data.value == apply_proc:
