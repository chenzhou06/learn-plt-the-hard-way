...
Empty string is treated as false, otherwise a string is true as a bool.
...
This method do not take a vector of points to ~PyObect~, instead it directly
take an integer.
...
convert these vector of strings to pointers and store them in a vector of
pointers to ~PyObject*~. The return value is ~PyList~ whose elements are ~PyStr~
s.
